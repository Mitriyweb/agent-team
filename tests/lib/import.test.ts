...
describe("import.ts", () => {
  ...
  it("imports from various tools", async () => {
    ...
    try {
      // Windsurf — use absolute paths for setup, chdir only for importConfig
      const wsDir = path.join(tmpDir, ".windsurf");
      ...
      fs.writeFileSync(path.join(tmpDir, ".windsurfrules"), "Legacy rules");

      process.chdir(tmpDir);
      await importConfig(".windsurf");
      expect(fs.existsSync(path.join(tmpDir, ".claude/rules/rule2.md"))).toBe(
        true,
      );
      expect(
      ...
      fs.writeFileSync(path.join(tmpDir, ".cursorrules"), "Cursor legacy");
      process.chdir(tmpDir);
      await importConfig(".cursor");
      expect(
        fs.readFileSync(path.join(tmpDir, "CLAUDE.md"), "utf-8"),
      ).toContain("Cursor body");
      expect(
        fs.readFileSync(path.join(tmpDir, "CLAUDE.md"), "utf-8"),
      ).toContain("Cursor legacy");
      ...
      process.chdir(tmpDir);
      await importConfig(".github");
      expect(
        fs.readFileSync(path.join(tmpDir, "CLAUDE.md"), "utf-8"),
      ).toContain("Copilot instructions");
      expect(
        fs.readFileSync(path.join(tmpDir, "CLAUDE.md"), "utf-8"),
      ).toContain("Copilot agent");
      ...
      process.chdir(tmpDir);
      await importConfig(path.join("other-project", ".claude"));
      expect(
        fs.readFileSync(path.join(tmpDir, "CLAUDE.md"), "utf-8"),
      ).toContain("Other CLAUDE.md");
      expect(
        fs.readFileSync(path.join(tmpDir, "CLAUDE.md"), "utf-8"),
      ).toContain("Other rule body");
    } finally {
      process.chdir(origCwd);
